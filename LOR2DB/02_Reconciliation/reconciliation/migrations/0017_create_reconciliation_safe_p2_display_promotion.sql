/* ============================================================================
Object group: Reconciliation-safe P2 display promotion
File:         0017_create_reconciliation_safe_p2_display_promotion.sql

Purpose:
  Install the internal P2 procedure that consumes only frozen candidates and
  effective append-only decisions for one reconciliation run.

Safety boundary:
  Installation defines objects only. It does not call P1/P2, record decisions,
  or modify ref.display data. P2 is not an operator entry point.

Revision history:
  2026-08-03  GAL / OpenAI  Initial reconciliation-gated P2.
============================================================================ */
...
'Internal reconciliation-gated P2. Revalidates exact raw source rows, rejects nonphysical names, preserves display_id and production-owned metadata, and applies only approved atomic groups.';

REVOKE EXECUTE ON PROCEDURE
    ref.p2_promote_display_from_reconciliation(bigint) FROM PUBLIC;

COMMENT ON SCHEMA ops IS
'Operational workflow and audit objects. Reconciliation engine revision 2026-08-03-safe-p2-v1 installed.';

COMMIT;

SELECT
    '2026-08-03-reconciliation-safe-p2-v1'::text AS installed_revision,
    to_regprocedure(
        'ref.p2_promote_display_from_reconciliation(bigint)'
    ) IS NOT NULL AS has_p2_procedure;
