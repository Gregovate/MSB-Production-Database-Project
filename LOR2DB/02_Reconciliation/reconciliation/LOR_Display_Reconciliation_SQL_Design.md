...
- **Repository Path:** `LOR2DB/Reconciliation/reconciliation/LOR_Display_Reconciliation_SQL_Design.md`
- **Document Type:** Database design specification
- **Status:** IMPLEMENTED AND PRODUCTION VALIDATED — Run 4 and lor2db acceptance complete
...
| Date | Author | Change |
|---|---|---|
| 2026-08-06 | GAL / OpenAI | Recorded installed effective-counter and true-no-op corrections, completed Run 4, and production acceptance of grants V0.2.0, backend V0.3.0, and the `lortodb.sheboyganlights.org` landing page. The manual SQL runbook is now fallback/recovery only. |
...
contract are maintained under `LOR2DB/Application/`. Production uses the
restricted application listener at `192.168.5.9:8784` behind the authenticated
reverse proxy. Run 4 and the landing page passed controlled production
acceptance on 2026-08-06. The manual SQL runbook remains only the fallback and
recovery path.
...
The implementation checklist formerly maintained here is complete. The
repository contains the installed reconciliation lifecycle, persistent
candidates and actions, reconciliation-safe P1-P4 promotion, post-write
validation, report publication, application grants, and secured operator
interface. Production Run 4 against snapshot 45 completed with validation
passed and zero exceptions; its immutable report and archive links were
verified through the production landing page on 2026-08-06.

Future changes must use a dedicated migration and matching validation script.
They must not rerun historical migrations or bypass the controlled Start,
decision, Finish/Cancel, validation, and report-publication boundaries.
