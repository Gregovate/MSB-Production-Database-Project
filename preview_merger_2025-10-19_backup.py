...
# # --- Canonical preview_historyDB path (the only DB we write to) ---

# HISTORY_DB_CANONICAL = Path(r"G:\Shared drives\MSB Database\Database Previews\reports\preview_history.db")


# def open_history_db(run_mode: str):
#     """
#     Dry-run: return (None, None) so we never mutate the DB.
#     Apply   : open R/W connection and ensure schema; return (conn, run_id).
#     """
#     import sqlite3, getpass, socket
#     from datetime import datetime
...
#         started_utc TEXT, mode TEXT, host TEXT, user TEXT
#     )""")
#     conn.execute("""CREATE TABLE IF NOT EXISTS apply_events(
#         id INTEGER PRIMARY KEY AUTOINCREMENT,
#         run_id INTEGER, ts_utc TEXT,
#         preview_name TEXT, author TEXT, revision TEXT,
#         src_path TEXT, dest_path TEXT, archived_prior TEXT,
#         action TEXT, note TEXT
#     )""")
#     conn.execute("""CREATE UNIQUE INDEX IF NOT EXISTS ux_apply_events_unique
#                     ON apply_events(ts_utc, preview_name, action, dest_path)""")

#     run_id = conn.execute(
#         "INSERT INTO runs(started_utc,mode,host,user) VALUES (?,?,?,?)",
#         (datetime.utcnow().isoformat(timespec="seconds"),
#          "apply", socket.gethostname(), getpass.getuser())
#     ).lastrowid
#     conn.commit()
#     return conn, run_id
...
def _get(obj, *keys):
    """
    Safe getter that works for objects (attributes) and dict-like.
    """
    cur = obj
    for k in keys:
        if cur is None:
...
def diff_core_fields(src: Path, dst: Path) -> tuple[bool, list[str]]:
    """
    Compare two previews at DB-meaningful granularity (PropClass + all grids).
    Returns (same, changed_fields[]). Robust against parser returning 0 rows.
    """
    try:
        src_rows = _snapshot_core(Path(src))
        dst_rows = _snapshot_core(Path(dst))
    except Exception as e:
        # Last-resort: any parser failure → rely on semantic hash
        try:
            sh_src = semantic_sha256_file(Path(src))
            sh_dst = semantic_sha256_file(Path(dst))
            same = (sh_src == sh_dst)
            return same, ([] if same else [f"ParserError→SemanticHash:{sh_dst[:8]}→{sh_src[:8]}"])
        except Exception:
            # Truly unknown: treat as different to be safe
            return False, ["ParserError:unknown"]

    sig_src, n_src = _sig(src_rows)
    sig_dst, n_dst = _sig(dst_rows)

    changed: list[str] = []
    if n_src != n_dst:
        changed.append(f"PropCount:{n_dst}→{n_src}")
    if sig_src != sig_dst:
        changed.append(f"PropListSig:{(sig_dst or '∅')}→{(sig_src or '∅')}")

    # If parser produced 0 rows on either side, sanity-fallback to semantic hash
    if (n_src == 0 or n_dst == 0) and Path(src).exists() and Path(dst).exists():
        try:
            sh_src = semantic_sha256_file(Path(src))
            sh_dst = semantic_sha256_file(Path(dst))
            if sh_src != sh_dst and f"PropListSig:{(sig_dst or '∅')}→{(sig_src or '∅')}" not in changed:
                changed.append(f"SemanticHash:{sh_dst[:8]}→{sh_src[:8]}")
        except Exception:
            # ignore; we already have counts/sigs
            pass

    same = (n_src == n_dst) and (sig_src == sig_dst)
    # If counts/sigs say "same" but semantic fallback found a diff, respect it
    if "SemanticHash:" in " ".join(changed):
        same = False
...
def write_dryrun_manifest_csv(
    ...
    _write_csv_atomic(
        ...
        [
            ...
            "AuthorNewer","CoreDifferent","CoreChangedFields"  # GAL 25-10-19
        ],
        needs_rows,
        attempts=3
    )
    ...
    ready_to_apply = [r for r in needs_rows if (r.get("ReadyToApply","").lower() == "yes")]
    would_stage_names = [r.get("PreviewName","") for r in ready_to_apply]

    # Console summary (short + readable)
    preview_list = ", ".join(would_stage_names[:8])
    if len(would_stage_names) > 8:
        preview_list += " …"
    print(f"[dry-run] WOULD STAGE: {len(ready_to_apply)} preview(s){(': ' + preview_list) if preview_list else ''}")
