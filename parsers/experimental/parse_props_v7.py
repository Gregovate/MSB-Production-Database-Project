## 2026-03-17  V7.0.0  (GAL)
# • Correct PropID identity model:
#   – Restore props.PropID to raw LOR PropClass.id from XML.
#   – Add props.ScopedPropID as synthetic PreviewId:PropID key for cross-preview uniqueness.
#   – Restore subProps.SubPropID and subProps.MasterPropId to raw XML ids.
#   – Add subProps.ScopedSubPropID and subProps.ScopedMasterPropId for scoped uniqueness.
# • Reason:
#   – Previous parser revisions used preview-scoped synthetic ids in PropID/SubPropID fields.
#   – That broke source-of-truth behavior for LORComment-driven display renames when previews
#     were copied and preview UUIDs changed.
# • Outcome:
#   – Raw LOR ids remain authoritative.
#   – Scoped ids remain available for uniqueness and internal joins.
#
# Revision History
# ----------------
## 2026-03-17  V7.0.0  (GAL)
...
# • Fix StageID parsing to support sub-stages and animation ordering:
#   – Support park stage tokens as NN or NNa (e.g., 07, 07a).
#   – Parse stage token from either:
#       * Prefix form: "07a-<Name> ..." / "07-<Name> ..."
#       * Embedded form: "... Stage 07a ..." / "... Stage 17 ..."
#   – Support "Show Animation" previews by extracting the first 1–2 digit number after
#     "Show Animation" and normalizing to 2 digits (e.g., 90 → "90").
#   – Remove legacy digits-only extraction and remove "animation returns full name" fallback.
# • Outcome: SQLite previews.StageID now reliably populates for sub-stages (07a) and
#   animation previews (90–94) without contaminating non-stage records.
#
## 2026-02-23  V6.8.2  (GAL)
# • Add preflight validation (NO DB writes) before setup_database():
#   – Validate Preview UUID uniqueness across ingest input set.
#   – Validate no duplicate PropClass GUIDs exist within a single preview file.
# • Improve DeviceType=None master guardrails:
#   – Detect and fail-fast when multiple MASTER inventory records share the same DisplayName
#     within a single preview (prevents ambiguous inventory masters).
# • Folder ingest visibility improvements:
#   – Ensure ingest explicitly reports how many .lorprev files are found in the production folder
#     (helps catch files skipped due to non-recursive folder logic).
#
# 2026-02-21  V6.8.1  (GAL)
# • Fix grid fallback bug (set vs dict) that could crash when Grid is missing.
# • Add normalize_grid() safety guard to enforce dict Grid objects (fail-fast with context).
# • Harden master insert paths for both single-grid and multi-grid LOR props:
#   – Ensure master rows carry full Network/UID/Start/End fields consistently.
#   – Collision-aware insert remains enforced (no silent overwrite).
#
...
def validate_previews(preview_root: Path):
    """
    Preflight validation (NO DB writes).
    - Ensure Preview UUID is unique across all .lorprev files in the ingest tree.
    - Ensure no duplicate PropClass GUIDs exist within a single preview file.

    Added 2026-02-23 (GAL) — catch basic problems before any DB writes.
    """
    import xml.etree.ElementTree as ET
    ...
    seen_preview_ids: dict[str, Path] = {}
    ...
    for file_path in lorprev_files:
        try:
            ...
            print(f"[FATAL] Could not parse preview XML: {file_path}")
            print(f"        {type(e).__name__}: {e}")
            raise SystemExit(2)

        preview_id = (root.get("id") or "").strip()
        if not preview_id:
            print(f"[FATAL] Preview missing id attribute: {file_path}")
            raise SystemExit(2)

        if preview_id in seen_preview_ids:
            print("[FATAL] Duplicate PreviewId detected BEFORE ingest.")
            print(f"        PreviewId: {preview_id}")
            print(f"        First file:  {seen_preview_ids[preview_id]}")
            print(f"        Second file: {file_path}")
            raise SystemExit(2)

        seen_preview_ids[preview_id] = file_path

        # Duplicate PropClass ids inside a single preview file
        prop_ids = []
        for prop in root.findall(".//PropClass"):
            rid = (prop.get("id") or "").strip()
            if rid:
                prop_ids.append(rid)

        if len(prop_ids) != len(set(prop_ids)):
            dupes = [k for k, v in Counter(prop_ids).items() if v > 1]
            print("[FATAL] Duplicate PropClass id(s) detected within a single preview file.")
            print(f"        File: {file_path}")
            for d in dupes[:10]:
                print(f"        PropClass id duplicated: {d}")
            if len(dupes) > 10:
                print(f"        ...and {len(dupes) - 10} more")
            raise SystemExit(2)

    print(f"[INFO] Preflight OK: {len(seen_preview_ids)} previews verified.")
...
# -----------------------------------------------------------------------------
# Grid normalization / safety guard 26/02/21 GAL
#
# Why this exists:
# - Downstream code expects Grid to be a dict with keys like "Network", "UID", etc.
# - If ANY upstream path accidentally returns a set (or other type), Python will crash
#   later with: TypeError: 'set' object is not subscriptable
#
# What this does:
# - Converts None/empty -> {}
# - Passes dict through unchanged
# - Hard-fails on wrong types with context that identifies the offending prop/comment
# -----------------------------------------------------------------------------
def normalize_grid(grid, *, ctx="") -> dict:
    if not grid:
        return {}
    if isinstance(grid, dict):
        return grid

    # Do not limp forward: crash immediately with useful context so we can fix the source.
    raise TypeError(
        f"Grid is {type(grid).__name__}, expected dict. ctx={ctx} value={repr(grid)[:200]}"
    )
...
# --- ID helpers -----UPDATED 26-03-18---------------------------------------
def scoped_id(preview_id: str, raw_id: str) -> str:
    """
    Synthetic preview-scoped key used only for cross-preview uniqueness.
    Do NOT use as source-of-truth prop identity.
    """
    return f"{preview_id}:{raw_id}"
# --- helpers ---------------------------------------------------------------
# --- Device Type = NONE  ---
...
def setup_database():
    ...
    # Create Props Table
    # GAL 26-03-17:
    #   PropID = raw LOR PropClass.id from XML (source-of-truth prop identity)
    ...
    # GAL 26-03-17:
    #   SubPropID = raw XML subprop id
    #   ScopedSubPropID = synthetic PreviewId:SubPropID
    #   MasterPropId = raw XML master prop id
    #   ScopedMasterPropId = synthetic PreviewId:MasterPropId for unique internal joins
    cursor.execute("""
        CREATE TABLE subProps (
            IntSubPropID INTEGER PRIMARY KEY AUTOINCREMENT,
            SubPropID TEXT,
            ScopedSubPropID TEXT UNIQUE,
            Name TEXT,
            LORComment TEXT,
            DeviceType TEXT,
            BulbShape TEXT,
            ...
            Lights INTEGER,
            PreviewId TEXT,
            FOREIGN KEY (ScopedMasterPropId) REFERENCES props (ScopedPropID),
            FOREIGN KEY (PreviewId) REFERENCES previews (id)
        );
    """)
...
# Park stage patterns
_STAGE_PREFIX_RE = re.compile(r'^\s*(\d{1,2})([a-z]?)\s*[-_]\s*', re.IGNORECASE)
_STAGE_TOKEN_RE  = re.compile(r'(?i)\bstage\s*0*(\d{1,2})([a-z]?)\b')
...
def extract_stage_id(name: str | None):
    ...
    Returns None if no valid stage token is found.
    """

    if not name:
        return None

    # 1) Park stage via prefix (07a-Whoville)
...
def process_none_props(preview_id, root, skip_display_names: set[str] | None = None):
    """
    DeviceType == "None" (inventory-only props)
    Policy:
      • If MasterPropId (XML) is empty  → this record is the INVENTORY MASTER → INSERT into props.
      • If MasterPropId is present      → this record is a linked unit        → IGNORE by default
        (or write to subProps if WRITE_LINKED_TO_SUBPROPS = True).
      • Masters may fan out into multiple physical instances using:
            MaxChannels if > 0
            else Parm2 if numeric and > 0
            else 1
    """
    ...
    # Helper functions for ID normalization and scoping updated 25-03-18 GAL
    def _raw(raw_id: str | None) -> str | None:
    ...
    def _scoped(preview_id: str, raw_id: str | None) -> str | None:
        ...
        if not raw_id:
            return None
        return scoped_id(preview_id, raw_id)
    ...
    for dn, m in masters_by_display.items():
        base_raw    = _raw(m["raw_id"]) or "NONE"
        ...
        for i in range(1, count + 1):
            inst_raw_id    = f"{base_raw}-{i:02d}"    if count > 1 else base_raw
            inst_scoped_id = f"{base_scoped}-{i:02d}" if count > 1 else base_scoped
            inst_comment   = f"{dn}-{i:02d}"          if count > 1 else dn
            try:
                insert_sql = """
                    INSERT INTO props (
                        PropID, ScopedPropID, Name, LORComment, DeviceType,
                        BulbShape, DimmingCurveName, MaxChannels,
                        CustomBulbColor, IndividualChannels, LegacySequenceMethod,
                        Opacity, MasterDimmable, PreviewBulbSize, MasterPropId, ScopedMasterPropId,
                        SeparateIds, StartLocation, StringType, TraditionalColors, TraditionalType,
                        EffectBulbSize, Tag,
                ...
                """
                params = (
                    inst_raw_id, inst_scoped_id, m["name"], inst_comment, "None",
                    m["bulb_shape"], m["dimming_curve_name"], m["max_ch"],
                    m["custom_bulb_color"], m["individual_ch"], m["legacy_method"],
                    m["opacity"], m["master_dimmable"], m["preview_bulb_size"], None, None,
                    m["separate_ids"], m["start_location"], m["string_type"], m["traditional_colors"], m["traditional_type"],
                    m["effect_bulb_size"], m["tag"],
                    m["parm1"], m["parm2"], m["parm3"], m["parm4"], m["parm5"], m["parm6"], m["parm7"], m["parm8"],
                    lights, preview_id
                )
                assert len(params) == 33, f"process_none_props master params={len(params)} expected 33"
                cur.execute(insert_sql, params)

            except sqlite3.IntegrityError as e:
                print(f"[ERROR] Duplicate ScopedPropID (None/MASTER): {inst_scoped_id} "
                      f"(PreviewId={preview_id}, Display='{inst_comment}') -> {e}")
                raise

        if DEBUG:
            mode = "MASTER-FANOUT" if count > 1 else "MASTER"
            print(f"[NONE->{mode}] raw='{base_raw}' scoped='{base_scoped}'  name='{m['name']}'  display='{dn}' x{count}")
    ...
    if WRITE_LINKED_TO_SUBPROPS and linked_rows:
        ...
        master_ids_by_raw = {
            masters_by_display[dn]["raw_id"]: {
                "raw": _raw(masters_by_display[dn]["raw_id"]) or "NONE",
                "scoped": _scoped(preview_id, masters_by_display[dn]["raw_id"]) or f"{preview_id}:NONE"
            }
        ...
        for r in linked_rows:
            ...
            sub_raw    = _raw(r["raw_id"]) or "NONE"
            sub_scoped = _scoped(preview_id, r["raw_id"]) or f"{preview_id}:NONE"
            master_ids = master_ids_by_raw[master_rec["raw_id"]]

            try:
                cur.execute("""
                    INSERT INTO subProps (
                        SubPropID, ScopedSubPropID,
                        MasterPropId, ScopedMasterPropId,
                ...
                """, (
                    sub_raw, sub_scoped,
                    master_ids["raw"], master_ids["scoped"],
                    r["name"], dn, "None",
                ...
                print(f"[ERROR] Duplicate ScopedSubPropID (None/LINK): {sub_scoped} "
                      f"(Master={master_ids['scoped']}, Display='{dn}') -> {e}")
                raise

            if DEBUG:
                print(f"[NONE->SUB] raw='{sub_raw}' scoped='{sub_scoped}'  name='{r['name']}'  display='{dn}'  master_raw='{master_ids['raw']}' master_scoped='{master_ids['scoped']}'")
    conn.commit()
    conn.close()
...
def process_dmx_props(preview_id, root):
    ...
    for comment, arr in groups.items():
        ...
        #   PropID = raw LOR PropClass.id
        #   ScopedPropID = synthetic PreviewId:PropID used only for cross-preview uniqueness
        #   MasterPropId / ScopedMasterPropId are NULL for root master rows
        insert_sql = """
            INSERT INTO props (
                PropID, ScopedPropID, Name, LORComment, DeviceType, BulbShape, DimmingCurveName,
                MaxChannels, CustomBulbColor, IndividualChannels, LegacySequenceMethod,
                Opacity, MasterDimmable, PreviewBulbSize, MasterPropId, ScopedMasterPropId,
                SeparateIds, StartLocation, StringType, TraditionalColors, TraditionalType,
                EffectBulbSize, Tag, Parm1, Parm2, Parm3, Parm4, Parm5, Parm6, Parm7, Parm8,
                Lights, PreviewId
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        params = (
            master["PropID"], master["ScopedPropID"], master["Name"], master["LORComment"], "DMX",
            ...
            master["Opacity"], master["MasterDimmable"], master["PreviewBulbSize"], None, None,
            master["SeparateIds"], master["StartLocation"], master["StringType"], master["TraditionalColors"], master["TraditionalType"],
        ...
        incoming_ctx = {
            ...
            "ScopedPropID": master.get("ScopedPropID"),
        ...
        if DEBUG:
            if ok:
                print(f"[DEBUG] (DMX) master → props INSERT: raw={master['PropID']} scoped={master['ScopedPropID']}  Display='{comment}'")
            else:
                print(f"[DEBUG] (DMX) master → props SKIP (collision/error): raw={master['PropID']} scoped={master['ScopedPropID']}  Display='{comment}'")
        ...
        for r in arr:
            for leg in r["Legs"]:
                ...
                if DEBUG:
                    print(f"[DEBUG] (DMX) +leg master_raw={master['PropID']} master_scoped={master['ScopedPropID']} U={leg['StartUniverse']} S={leg['StartChannel']} E={leg['EndChannel']}")
    conn.commit()
    conn.close()
...
def process_lor_props(preview_id, root):
    ...
    for prop in root.findall(".//PropClass"):
        ...
        if "spare" in name.lower():
            ...
            #   PropID = raw LOR PropClass.id
            #   ScopedPropID = synthetic PreviewId:PropID used only for cross-preview uniqueness
            #   MasterPropId / ScopedMasterPropId are NULL for root SPARE rows
            insert_sql = """
                INSERT INTO props (
                    PropID, ScopedPropID, Name, LORComment, DeviceType, BulbShape, DimmingCurveName, MaxChannels,
                    CustomBulbColor, IndividualChannels, LegacySequenceMethod, Opacity, MasterDimmable,
                    PreviewBulbSize, MasterPropId, ScopedMasterPropId, SeparateIds, StartLocation, StringType,
                    TraditionalColors, TraditionalType, EffectBulbSize, Tag, Parm1, Parm2, Parm3, Parm4, Parm5,
                    Parm6, Parm7, Parm8, Lights, Network, UID, StartChannel, EndChannel, Unknown, Color, PreviewId
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """
            params = (
                prop_id_raw, prop_id_scoped, name, prop.get("Comment", ""), "LOR",
            ...
            if DEBUG:
                if ok:
                    print(f"[DEBUG] (LOR single) SPARE -> props INSERT: raw={prop_id_raw} scoped={prop_id_scoped} '{name}'")
                else:
                    print(f"[DEBUG] (LOR single) SPARE -> props SKIP (collision/error): raw={prop_id_raw} scoped={prop_id_scoped} '{name}'")
    # -----------------------------------------------------------------------
    ...
    for sp, master in manuals_same:
        sub_id_raw       = sp.get("id") or ""
        sub_id_scoped    = scoped_id(preview_id, sub_id_raw)
        master_id_raw    = (master.get("id") or "") if master is not None else None
        master_id_scoped = scoped_id(preview_id, master_id_raw) if master_id_raw else None
        g = grid_or(sp, grid_or(master))
        # Process_LOR_Props PASS 0B: MANUAL SUBPROPS (MasterPropId set)
        # 2026-03-18 GAL:
        #   SubPropID = raw LOR subprop id
        #   ScopedSubPropID = synthetic PreviewId:SubPropID used only for cross-preview uniqueness
        #   MasterPropId = raw LOR master prop id
        #   ScopedMasterPropId = synthetic PreviewId:MasterPropId used for scoped joins
        insert_sql = """
            INSERT INTO subProps (
                SubPropID, ScopedSubPropID, Name, LORComment, DeviceType, BulbShape, Network, UID, StartChannel,
                ...
                MasterPropId, ScopedMasterPropId, SeparateIds, StartLocation, StringType, TraditionalColors, TraditionalType,
                EffectBulbSize, Tag, Parm1, Parm2, Parm3, Parm4, Parm5, Parm6, Parm7, Parm8, Lights, PreviewId
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        params = (
            sub_id_raw, sub_id_scoped, sp.get("Name",""), sp.get("Comment",""), "LOR", sp.get("BulbShape"),
            ...
            master_id_raw, master_id_scoped, sp.get("SeparateIds"), sp.get("StartLocation"), sp.get("StringType"),
        ...
        incoming_ctx = {
            "SubPropID":    sub_id_raw,
            "ScopedSubPropID": sub_id_scoped,
            "Name":         sp.get("Name",""),
            ...
            "MasterPropId": master_id_raw,
            "ScopedMasterPropId": master_id_scoped,
        }
        ok = safe_insert_subprop(cursor, insert_sql, params, incoming_ctx)
        if DEBUG:
            if ok:
                print(f"[DEBUG] (LOR manual subprop) INSERT: raw={sub_id_raw} scoped={sub_id_scoped}  MasterRaw='{master_id_raw}' MasterScoped='{master_id_scoped}'")
            else:
                print(f"[DEBUG] (LOR manual subprop) SKIP (collision/error): raw={sub_id_raw} scoped={sub_id_scoped}  MasterRaw='{master_id_raw}' MasterScoped='{master_id_scoped}'")
    ...
    for new_comment, manual_nodes in changed_by_comment.items():
        ...
        g_master = grid_or(new_master)
        new_master_id_raw = new_master.get("id") or ""
        new_master_id = scoped_id(preview_id, new_master_id_raw)

        # Insert new master for a changed Display_Name group
        # 2026-03-18 GAL:
        #   PropID = raw LOR PropClass.id
        #   ScopedPropID = synthetic PreviewId:PropID used only for cross-preview uniqueness
        #   MasterPropId / ScopedMasterPropId are NULL for root master rows
        ...
        """
        params = (
            new_master_id_raw, new_master_id, new_master.get("Name",""), new_comment, "LOR",
            ...
            new_master.get("PreviewBulbSize"), None, None,
            new_master.get("SeparateIds"), new_master.get("StartLocation"), new_master.get("StringType"),
            new_master.get("TraditionalColors"), new_master.get("TraditionalType"), new_master.get("EffectBulbSize"),
            new_master.get("Tag"), new_master.get("Parm1"), new_master.get("Parm2"),
        ...
        incoming_ctx = {
            "PropID":       new_master_id_raw,
            "ScopedPropID": new_master_id,
        ...
        if DEBUG:
            if ok:
                print(f"[DEBUG] (LOR master new) -> props INSERT: raw={new_master_id_raw} scoped={new_master_id}  Display='{new_comment}'")
            else:
                print(f"[DEBUG] (LOR master new) -> props SKIP (collision/error): raw={new_master_id_raw} scoped={new_master_id}  Display='{new_comment}'")
        ...
        for node in full_group:
            if node is new_master:
                continue
            g = grid_or(node)
            sub_id_raw = node.get("id") or ""
            sub_id = scoped_id(preview_id, sub_id_raw)

            # Insert grouped subprop under the new master for a changed Display_Name group
            # 2026-03-18 GAL:
            #   SubPropID = raw LOR subprop id
            #   ScopedSubPropID = synthetic PreviewId:SubPropID used only for cross-preview uniqueness
            #   MasterPropId = raw LOR master prop id
            ...
            """
            params = (
                sub_id_raw, sub_id, node.get("Name",""), new_comment, "LOR", node.get("BulbShape"),
                g["Network"], g["UID"], g["StartChannel"], g["EndChannel"], g["Unknown"], g["Color"],
                ...
                new_master_id_raw, new_master_id, node.get("SeparateIds"), node.get("StartLocation"), node.get("StringType"),
            ...
            incoming_ctx = {
                ...
                "StartChannel":      g.get("StartChannel",""),
                "EndChannel":        g.get("EndChannel",""),
                "MasterPropId":      new_master_id_raw,
                "ScopedMasterPropId": new_master_id,
            }
            ok = safe_insert_subprop(cursor, insert_sql, params, incoming_ctx)
            if DEBUG:
                if ok:
                    print(f"[DEBUG] (LOR group sub) -> subProps INSERT: raw={sub_id_raw} scoped={sub_id}  MasterRaw='{new_master_id_raw}' MasterScoped='{new_master_id}' Display='{new_comment}'")
                else:
                    print(f"[DEBUG] (LOR group sub) -> subProps SKIP (collision/error): raw={sub_id_raw} scoped={sub_id}  MasterRaw='{new_master_id_raw}' MasterScoped='{new_master_id}' Display='{new_comment}'")
    ...
    for display_name, group in props_grouped_by_comment.items():
        ...
        # master_id is used throughout this block; define it immediately after selecting master.
        master_id = master["PropID_scoped"]

        # ---------------------------------------------------------------------
        # Master grid safety:
        # - Downstream code expects Grid to be a dict (subscriptable).
        # - normalize_grid() guarantees dict-or-{} and fails loudly with context
        #   if something upstream returns the wrong type.
        # ---------------------------------------------------------------------
        m_grid = normalize_grid(
            master.get("Grid"),
            ctx=f"single-grid master preview={preview_id} master_scoped_id={master_id} comment={display_name}"
        )

        # Ensure all expected keys exist (missing -> None) so params tuple is stable.
        m_grid_full = {
            "Network": None,
            "UID": None,
            "StartChannel": None,
        ...
        """
        params = (
            master["PropID_raw"], master["PropID_scoped"], master["Name"], display_name, master["DeviceType"], master["BulbShape"],
            ...
            master["MasterDimmable"], master["PreviewBulbSize"], None, None,
            ...
            m_grid_full["Network"], m_grid_full["UID"], m_grid_full["StartChannel"], m_grid_full["EndChannel"],
            m_grid_full["Unknown"], m_grid_full["Color"], master["PreviewId"]
        )
        incoming_ctx = {
            "PropID":       master["PropID_raw"],
            "ScopedPropID": master_id,
            "Name":         master.get("Name",""),
            "LORComment":   display_name,
            "PreviewId":    master.get("PreviewId"),
            "DeviceType":   master.get("DeviceType",""),
            "Network":      (m_grid_full or {}).get("Network",""),
            "UID":          (m_grid_full or {}).get("UID",""),
            "StartChannel": (m_grid_full or {}).get("StartChannel",""),
            "EndChannel":   (m_grid_full or {}).get("EndChannel",""),
        }
        ok = safe_insert_prop(cursor, insert_sql, params, incoming_ctx)
        if DEBUG:
            if ok:
                print(f"[DEBUG] (LOR single) MASTER -> props INSERT: raw={master['PropID_raw']} scoped={master_id} '{display_name}' Start={m_grid_full['StartChannel']}")
            else:
                print(f"[DEBUG] (LOR single) MASTER -> props SKIP (collision/error): raw={master['PropID_raw']} scoped={master_id} '{display_name}' Start={m_grid_full['StartChannel']}")
        ...
        for rec in group:
            ...
            sub_id_raw = rec["PropID_raw"]
            sub_id = rec["PropID_scoped"]

            # AUTO-grouped subprop under selected master
            # 2026-03-18 GAL:
            #   SubPropID = raw LOR subprop id
            #   ScopedSubPropID = synthetic PreviewId:SubPropID used only for cross-preview uniqueness
            #   MasterPropId = raw LOR master prop id
            ...
            """
            params = (
                sub_id_raw, sub_id, rec["Name"], rec["LORComment"], rec["DeviceType"], rec["BulbShape"],
                g["Network"], g["UID"], g["StartChannel"], g["EndChannel"], g["Unknown"], g["Color"],
                ...
                master["PropID_raw"], master_id, rec["SeparateIds"], rec["StartLocation"], rec["StringType"], rec["TraditionalColors"],
            ...
            incoming_ctx = {
                ...
                "MasterPropId":      master["PropID_raw"],
                "ScopedMasterPropId": master_id,
            }
            ok = safe_insert_subprop(cursor, insert_sql, params, incoming_ctx)
            if DEBUG:
                if ok:
                    print(f"[DEBUG] (LOR single) AUTO -> subProps INSERT: parent_raw={master['PropID_raw']} parent_scoped={master_id} sub_raw={sub_id_raw} sub_scoped={sub_id} Start={g['StartChannel']}")
                else:
                    print(f"[DEBUG] (LOR single) AUTO -> subProps SKIP (collision/error): parent_raw={master['PropID_raw']} parent_scoped={master_id} sub_raw={sub_id_raw} sub_scoped={sub_id} Start={g['StartChannel']}")
...
def process_lor_multiple_channel_grids(preview_id, root):
    ...
    groups = {}  # raw PropID -> list of flat grid entries for that prop
    for prop in root.findall(".//PropClass"):
        if prop.get("DeviceType") != "LOR":
            continue
        ch_raw = (prop.get("ChannelGrid") or "").strip()
    ...
    def _pair_key_grid(d):
        g = d.get("Grid") or {}
        uid = (g.get("UID") or "").strip().upper()
        uid_key = uid if uid else "ZZ"
        sc = g.get("StartChannel")
        sc_key = sc if sc is not None else 10**9
        rid = scoped_id(preview_id, d.get("raw_id") or "") if d.get("raw_id") else (d.get("Name") or "")
        return (uid_key, sc_key, rid)
    ...
    for raw_id_key, items in groups.items():
        ...
        # (leave logic, but from here on treat master_raw_id as raw and master_id as scoped) 26-03-18 GAL

        # ---------------------------------------------------------------------
        # Fields for master props row 2026-02-21 GAL
        #
        # WHY normalize_grid():
        # - m_grid MUST be a dict, because we subscript it below (m_grid_full["Network"], etc.)
        # - If any upstream path returns a set (or other type), we fail here with context,
        #   instead of crashing later with the vague "set is not subscriptable".
        # ---------------------------------------------------------------------
        m_grid = normalize_grid(
            master.get("Grid"),
            ctx=f"multi-grid master preview={preview_id} master_id={master_id} comment={comment}"
        )

        # Ensure expected keys exist; missing keys become None (keeps SQL tuple stable).
        m_grid_full = {
            "Network": None,
            "UID": None,
            "StartChannel": None,
        ...
        props_row = (
            master_raw_id,          # TRUE LOR PropID
            master_id,              # Synthetic scoped ID
            master["Name"],
            comment,
            "LOR",
            master["BulbShape"],
            master["DimmingCurveName"],
            master["MaxChannels"],
            master["CustomBulbColor"],
            master["IndividualChannels"],
            master["LegacySequenceMethod"],
            master["Opacity"],
            master["MasterDimmable"],
            master["PreviewBulbSize"],
            None,                   # MasterPropId (root master)
            None,                   # ScopedMasterPropId
            master["SeparateIds"],
            master["StartLocation"],
            master["StringType"],
            master["TraditionalColors"],
            master["TraditionalType"],
            master["EffectBulbSize"],
            master["Tag"],
            master["Parm1"],
            master["Parm2"],
            master["Parm3"],
            ...
            m_grid_full["Network"],
            m_grid_full["UID"],
            m_grid_full["StartChannel"],
            m_grid_full["EndChannel"],
            m_grid_full["Unknown"],
            m_grid_full["Color"],
            preview_id
        )
        # Insert master props row for a multi-channel-grid LOR prop
        # 2026-03-18 GAL:
        #   PropID = raw LOR PropClass.id
        #   ScopedPropID = synthetic PreviewId:PropID used only for cross-preview uniqueness
        #   MasterPropId / ScopedMasterPropId are NULL for root master rows
        #   Full network/channel fields are written on the master row
        insert_sql = """
            INSERT INTO props (
                PropID, ScopedPropID, Name, LORComment, DeviceType, BulbShape, DimmingCurveName, MaxChannels,
                CustomBulbColor, IndividualChannels, LegacySequenceMethod, Opacity, MasterDimmable,
                PreviewBulbSize, MasterPropId, ScopedMasterPropId, SeparateIds, StartLocation, StringType,
                TraditionalColors, TraditionalType, EffectBulbSize, Tag, Parm1, Parm2, Parm3, Parm4, Parm5, Parm6,
                Parm7, Parm8, Lights, Network, UID, StartChannel, EndChannel, Unknown, Color, PreviewId
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        ...
        name_for_ctx = ""
        try:
            if isinstance(props_row, (list, tuple)) and len(props_row) > 2:
                name_for_ctx = props_row[2] or ""
        except Exception:
            pass
        incoming_ctx = {
            "PropID":       master_raw_id,
            "ScopedPropID": master_id,
            "Name":         name_for_ctx,
            "LORComment":   comment,
            "PreviewId":    preview_id,
            "DeviceType":   "LOR",
            "Network":      m_grid_full.get("Network", ""),
            "UID":          m_grid_full.get("UID", ""),
            "StartChannel": m_grid_full.get("StartChannel", ""),
            "EndChannel":   m_grid_full.get("EndChannel", ""),
        }

        ok = safe_insert_prop(cursor, insert_sql, params, incoming_ctx)
        if DEBUG:
            start = m_grid_full.get("StartChannel")
            uid   = m_grid_full.get("UID")
            if ok:
                print(f"[DEBUG] (LOR multi) MASTER → props INSERT raw={master_raw_id} scoped={master_id}  comment={comment}  start={start} uid={uid}")
            else:
                print(f"[DEBUG] (LOR multi) MASTER → props SKIP (collision/error) raw={master_raw_id} scoped={master_id}  comment={comment}  start={start} uid={uid}")

        # Insert remaining grids into subProps
        # 2026-03-18 GAL:
        #   SubPropID = raw synthetic subprop id built from raw master id + UID + StartChannel
        #   ScopedSubPropID = synthetic PreviewId-scoped subprop id built from scoped master id + UID + StartChannel
        #   MasterPropId = raw LOR master prop id
        #   ScopedMasterPropId = synthetic PreviewId:MasterPropId used for scoped joins
        #   Skip subprop emission for aggregate one-plug RGB/RGBW units
        if not is_aggregate:
            ...
            for d in items_sorted[1:]:
                ...
                sub_id_raw = f"{master_raw_id}-{uid}-{start:02d}"
                ...
                """
                params = (
                    sub_id_raw, sub_id, sub_name, comment, "LOR", d["BulbShape"],
                    ...
                    d["MaxChannels"], d["Opacity"], d["MasterDimmable"], d["PreviewBulbSize"], None,
                    master_raw_id, master_id, d["SeparateIds"], d["StartLocation"], d["StringType"], d["TraditionalColors"],
                ...
                if DEBUG:
                    if ok:
                        print(f"[DEBUG] (LOR multi) SUB  → subProps INSERT raw={sub_id_raw} scoped={sub_id}  parent_raw={master_raw_id} parent_scoped={master_id}  start={g['StartChannel']} uid={uid}")
                    else:
                        print(f"[DEBUG] (LOR multi) SUB  → subProps SKIP (collision/error) raw={sub_id_raw} scoped={sub_id}  parent_raw={master_raw_id} parent_scoped={master_id}  start={g['StartChannel']} uid={uid}")
        else:
            if DEBUG:
...
def process_folder(folder_path):
    ...
    lorprevs = [f for f in files if f.lower().endswith(".lorprev")]

    print(f"[INFO] Found {len(lorprevs)} .lorprev files in: {folder_path}")

    if not lorprevs:
        print(f"[WARNING] No .lorprev files found in folder: {folder_path}")
        return

    for file_name in sorted(lorprevs):
