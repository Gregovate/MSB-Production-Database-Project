# MSB Procedures structure updater launcher for Windows
#
# Default behavior is DRY-RUN. Pass --apply only after reviewing proposed
# creates/deletes. This updater only touches the Procedures tree at existing
# resolved Stage/Sub-stage/Scene scopes.

$ErrorActionPreference = 'Stop'

$RepoRoot = $PSScriptRoot
$UpdaterScript = Join-Path $RepoRoot 'Docs\01_LOR_System\02_Data_Extraction\Folder_Alignment\update_procedure_structure.py'

if (-not (Test-Path -LiteralPath $UpdaterScript -PathType Leaf)) {
    Write-Error "Procedures structure updater not found: $UpdaterScript"
    exit 2
}
...
Write-Host "[INFO] Procedures structure updater: $UpdaterScript"
