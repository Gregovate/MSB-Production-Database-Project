...
- Normal Preview and Scene background images belong in the appropriate `PreviewBackground` folder.
- Field wiring images belong in the appropriate `Wiring\BackgroundStage` or `Wiring\MusicalStage` folder.
- Do not create a Google Drive folder just because an item exists in LOR. Use the folder structure already established for that Stage, Scene, group, or Display.
...
The documents above are operator instructions. They explain how to **use** the Preview system.

Engineering documents that explain how the parser, Folder Alignment, FormView, database, and other internal systems work are kept separately so operator instructions can stay simple.
...
- 2026-08-22 — Rewritten as a plain-language operator portal. Technical implementation detail moved out of the normal operator path while keeping engineering links available separately.
- 2026-08-17 — Reconciled Preview Authoring with the current folder hierarchy, Master Musical Preview, `UserPreviewStaging`, and wiring workflow.
