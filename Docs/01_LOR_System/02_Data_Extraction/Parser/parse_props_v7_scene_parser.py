# MSB Database — LOR Preview Parser (V7)
#
...
# Purpose
# -------
# Parse Light-O-Rama (.lorprev) preview files and materialize:
#
#   • previews
#   • props
#   • subProps
...
# • Records parser version, start/completion timestamps, actor, host, source preview
#   folder, SQLite database path, and execution status for PostgreSQL ingest handoff.
# • parser_run is dropped and recreated with every parser execution; only the current
#   SQLite snapshot provenance is retained.
#
...
#   subprops, and duplicate-master demotion.
# • Added a fail-fast raw PropClass identity audit; parser completion now requires
#   every props and subProps row to contain a nonblank RawPropID.
#
## 2026-07-30  V7.0.3  (GAL)
# • Resolve positional scene XML rows to the display records materialized by the parser.
# • Expand DeviceType=None inventory fan-out into scene membership for every physical row.
# • Consolidate grouped DMX XML components onto the canonical DMX display master.
# • Exclude blank-comment None layout helpers from display-level scene membership.
# • Fail immediately on any unresolved nonblank scene row.
# • Added a final scene membership integrity audit; parser completion now requires zero
#   scene_lor_props rows missing from both props and subProps.
#
...
def setup_database():
    ...
    # Create Parser Run Table
    # GAL 2026-08-03:
    # Current parser execution metadata for PostgreSQL ingest handoff.
    # The SQLite database is rebuilt every run, so this table intentionally
    # retains only the provenance of the current snapshot.
    cursor.execute("""
...
def process_scene_lor_props(preview_id: str, preview_stage_id: str | None, root):
    """
    Build current positional Scene -> parsed-display membership for one preview.
    ...
    Rules:
      ...
      - Resolve XML rows to the records actually produced by the parser:
          * exact props/subProps ID for ordinary LOR rows;
          * all suffixed props rows for DeviceType=None inventory fan-out;
          * the canonical props row for DMX rows grouped by LORComment;
          * the channel-owned row when a None row was intentionally skipped
            because LOR/DMX owns the same display name.
      - Blank-comment None rows are presentation/layout helpers, not database
        displays, and are intentionally omitted.
      - An unresolved nonblank row is a parser error; never leave a silent
        orphan in scene_lor_props.
    ...
    ordinal = 0
    inserted = 0
    blank_helpers_skipped = 0
    unresolved = []

    def resolve_scene_prop_ids(element):
        """Return the parsed props/subProps IDs represented by one XML row."""
        raw_prop_id = (element.get("id") or "").strip()
        if not raw_prop_id:
            return []

        xml_prop_id = scoped_id(preview_id, raw_prop_id)
        comment = (element.get("Comment") or "").strip()

        # Ordinary LOR master/subprop rows retain the scoped XML ID.
        cursor.execute("""
            SELECT PropID
            FROM props
        ...
        """, (preview_id, xml_prop_id, preview_id, xml_prop_id))
        exact = [row[0] for row in cursor.fetchall()]
        if exact:
            return exact

        # DeviceType=None inventory masters may fan out to base-01..base-NN.
        cursor.execute("""
            SELECT PropID
            FROM props
            WHERE PreviewId = ?
              AND PropID GLOB ?
            ORDER BY PropID
        """, (preview_id, f"{xml_prop_id}-[0-9][0-9]"))
        fanout = [row[0] for row in cursor.fetchall()]
        if fanout:
            return fanout

        # DMX components are grouped to one canonical master by display name.
        # This also resolves None rows skipped because a channel-bearing record
        # owns the same display name.
        if comment:
            cursor.execute("""
                SELECT PropID
                FROM props
                ...
                ORDER BY
                    ...
                    END,
                    COALESCE(UID, 'ZZ'),
                    COALESCE(StartChannel, 1000000000),
                    PropID
                LIMIT 1
            """, (preview_id, comment))
    ...
    for element in root.iter():
        ...
        resolved_prop_ids = resolve_scene_prop_ids(element)
        if not resolved_prop_ids:
            comment = (element.get("Comment") or "").strip()
            device_type = (element.get("DeviceType") or "").strip()
            if not comment and device_type == "None":
                blank_helpers_skipped += 1
                continue
            unresolved.append({
        ...
        for prop_id in resolved_prop_ids:
            ...
            cursor.execute("""
                INSERT OR IGNORE INTO scene_lor_props (
                    PreviewId,
                    PropID,
                    RawPropID,
                    SceneID,
    ...
    if unresolved:
        ...
        details = "\n".join(
            "  - "
            f"SceneID={item['SceneID']} RawPropID={item['RawPropID']} "
            f"DeviceType={item['DeviceType']!r} Name={item['Name']!r} "
            f"Comment={item['Comment']!r}"
            for item in unresolved
        )
        conn.close()
        raise RuntimeError(
            f"Unresolved nonblank scene PropClass rows for PreviewId={preview_id}:\n"
            f"{details}"
        )
    ...
    INFO(
        f"Scene display memberships inserted: {inserted}; "
        f"blank layout helpers skipped: {blank_helpers_skipped}",
        ctx=preview_id
    )
...
def _parser_actor() -> str:
    """Return the operating-system user running the parser."""
    return (
        os.environ.get("USERNAME")
        or os.environ.get("USER")
        or getpass.getuser()
...
def _parser_host() -> str:
    """Return the workstation/host running the parser."""
    return (
        os.environ.get("COMPUTERNAME")
        or socket.gethostname()
        or platform.node()
...
def start_parser_run() -> None:
    ...
    setup_database() has already rebuilt parser_run, so no historical rows
    exist in this disposable SQLite snapshot.
    """
...
def audit_raw_prop_identity(db_file: str):
    ...
    try:
        rows = conn.execute("""
            SELECT 'props' AS Source, PropID AS ItemID
            FROM props
        ...
        if rows:
            details = "\n".join(
                f"  - Source={source} ItemID={item_id}"
                for source, item_id in rows
            )
            raise RuntimeError(
                f"Raw PropClass identity audit failed: {len(rows)} row(s) missing RawPropID.\n"
                f"{details}"
            )

        print("[OK] Raw PropClass identity audit: 0 missing RawPropID values.")
    finally:
        conn.close()

def audit_scene_membership_integrity(db_file: str):
    """
    Fail fast if a scene membership does not resolve to props or subProps.

    scene_lor_props is intentionally polymorphic, so SQLite cannot express
    this rule as one ordinary foreign key. Enforce it explicitly before any
    reports or downstream ingest can treat the database as complete.
    """
    conn = sqlite3.connect(db_file)
    try:
        rows = conn.execute("""
            ...
            WHERE NOT EXISTS (
                SELECT 1
                FROM props p
                WHERE p.PropID = slp.PropID
            )
              AND NOT EXISTS (
                SELECT 1
                FROM subProps sp
                WHERE sp.SubPropID = slp.PropID
            )
            ORDER BY pv.Name, s.Name, slp.ScenePropOrdinal
        """).fetchall()

        if rows:
            details = "\n".join(
                "  - "
                f"Preview={preview_name!r} Scene={scene_name!r} "
                f"RawPropID={raw_prop_id} PropID={prop_id}"
                for preview_name, scene_name, raw_prop_id, prop_id in rows
            )
            raise RuntimeError(
                f"Scene membership integrity audit failed: "
                f"{len(rows)} unresolved row(s).\n{details}"
            )

        print("[OK] Scene membership integrity audit: 0 unresolved rows.")
    finally:
        conn.close()
    ...
    for dn, m in masters_by_display.items():
        ...
        for i in range(1, count + 1):
            ...
            try:
                cur.execute("""
                    INSERT INTO props (
                        PropID, RawPropID, Name, LORComment, DeviceType,
            ...
            try:
                cur.execute("""
                    INSERT INTO subProps (
                        SubPropID, RawPropID, MasterPropId,
                        Name, LORComment, DeviceType,
                        Parm1, Parm2, Parm3, Parm4, Parm5, Parm6, Parm7, Parm8,
                        PreviewId
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    sub_scoped, r["raw_id"], master_scoped,
                    r["name"], dn, "None",
...
def process_dmx_props(preview_id, root):
    ...
    for comment, arr in groups.items():
        ...
        insert_sql = """
            INSERT INTO props (
                PropID, RawPropID, Name, LORComment, DeviceType, BulbShape, DimmingCurveName,
        ...
        """
        params = (
            master["PropID"], master["RawID"], master["Name"], master["LORComment"], "DMX",
...
def process_lor_props(preview_id, root):
    ...
    for prop in root.findall(".//PropClass"):
        ...
        if "spare" in name.lower():
            ...
            insert_sql = """
                INSERT INTO props (
                    PropID, RawPropID, Name, LORComment, DeviceType, BulbShape, DimmingCurveName, MaxChannels,
            ...
            """
            params = (
                prop_id_scoped, raw_id, name, prop.get("Comment", ""), "LOR",
    ...
    for sp, master in manuals_same:
        ...
        insert_sql = """
            INSERT INTO subProps (
                SubPropID, RawPropID, Name, LORComment, DeviceType, BulbShape, Network, UID, StartChannel,
        ...
        """
        params = (
            sub_id_scoped, sp.get("id") or "", sp.get("Name",""), sp.get("Comment",""), "LOR", sp.get("BulbShape"),
    ...
    for new_comment, manual_nodes in changed_by_comment.items():
        ...
        """
        params = (
            new_master_id, new_master.get("id") or "", new_master.get("Name",""), new_comment, "LOR",
        ...
        for node in full_group:
            ...
            """
            params = (
                sub_id, node.get("id") or "", node.get("Name",""), new_comment, "LOR", node.get("BulbShape"),
                g["Network"], g["UID"], g["StartChannel"], g["EndChannel"], g["Unknown"], g["Color"],
    ...
    for display_name, group in props_grouped_by_comment.items():
        ...
        """
        params = (
            master_id, master["PropID_raw"], master["Name"], display_name, master["DeviceType"], master["BulbShape"],
        ...
        if DEBUG:
            if ok:
                print(f"[DEBUG] (LOR single) MASTER -> props INSERT: {master_id} '{display_name}' Start={m_grid_full['StartChannel']}")
            else:
                print(f"[DEBUG] (LOR single) MASTER -> props SKIP (collision/error): {master_id} '{display_name}' Start={m_grid_full['StartChannel']}")
        ...
        for rec in group:
            ...
            """
            params = (
                sub_id, rec["PropID_raw"], rec["Name"], rec["LORComment"], rec["DeviceType"], rec["BulbShape"],
                g["Network"], g["UID"], g["StartChannel"], g["EndChannel"], g["Unknown"], g["Color"],
...
def process_lor_multiple_channel_grids(preview_id, root):
    ...
    for raw_id_key, items in groups.items():
        ...
        props_row = (
            master_id, master_raw_id, master["Name"], comment, "LOR", master["BulbShape"], master["DimmingCurveName"],
            ...
            m_grid_full["Network"], m_grid_full["UID"], m_grid_full["StartChannel"], m_grid_full["EndChannel"],
            m_grid_full["Unknown"], m_grid_full["Color"],
            preview_id
        )
        ...
        if DEBUG:
            ...
            uid   = m_grid_full.get("UID")
            if ok:
                print(f"[DEBUG] (LOR multi) MASTER → props INSERT id={master_id}  comment={comment}  start={start} uid={uid}")
            else:
                print(f"[DEBUG] (LOR multi) MASTER → props SKIP (collision/error) id={master_id}  comment={comment}  start={start} uid={uid}")
        ...
        if not is_aggregate:
            ...
            for d in items_sorted[1:]:
                ...
                """
                params = (
                    sub_id, d["raw_id"], sub_name, comment, "LOR", d["BulbShape"],
...
def process_file(file_path):
    ...
    if preview is not None:
        ...
        # Resolve scene membership only after all parser-owned props, subProps,
        # DMX masters, and None inventory fan-out rows exist.
...
def collapse_duplicate_masters(db_file: str):
    ...
    try:
        ...
        INSERT OR REPLACE INTO subProps (
            SubPropID, RawPropID, Name, LORComment, DeviceType, BulbShape,
        ...
        SELECT
            p.PropID, p.RawPropID, p.Name, p.LORComment, p.DeviceType, p.BulbShape,
