/* ============================================================================
Controller Inventory — simplify Directus Controller metadata PREVIEW
Issue: #110

ROLLBACK ONLY. Leaves no persistent metadata or privilege change.
...
Purpose:
  ...
  booleans, metadata fields, and Controller CRUD permissions.
...
Preview changes:
  ...
  - preserve all PostgreSQL assignment/history rows unchanged.
============================================================================ */

BEGIN;

\echo '=== SIMPLIFICATION PREVIEW preflight ==='
DO $preflight$
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Expected controller.display_assignments metadata is not present';
    END IF;
END
$preflight$;

\echo '=== SIMPLIFICATION PREVIEW current complex aliases ==='
SELECT field, special, interface, "group"
...
WHERE (many_collection='controller_display' AND many_field='controller_id' AND one_collection='controller')
   OR (many_collection='controller_firmware_history' AND many_field='controller_id' AND one_collection='controller')
ORDER BY id;

\echo '=== SIMPLIFICATION PREVIEW remove reverse workspaces ==='
...
\echo '=== SIMPLIFICATION PREVIEW restore Directus no-delete boundary ==='
...
\echo '=== SIMPLIFICATION PREVIEW proof ==='
SELECT
    EXISTS (
        SELECT 1 FROM public.directus_fields
        WHERE collection='controller' AND field='display_assignments'
...
\echo '=== SIMPLIFICATION PREVIEW expected invariant ==='
DO $assertions$
BEGIN
    IF EXISTS (
        SELECT 1 FROM public.directus_fields
...
\echo '=== SIMPLIFICATION PREVIEW PASS — rolling back ==='
ROLLBACK;
...
SELECT
    EXISTS (
        SELECT 1 FROM public.directus_fields
        WHERE collection='controller' AND field='display_assignments'
    ) AS display_assignments_restored,
    has_table_privilege('directus_app','ref.controller_display','DELETE') AS assignment_delete_restored;
