...
def test_controller_search_exposes_stage_match_context_without_changing_filter():
    ...
    script = (BASE_DIR / "static" / "controllers_search_context.js").read_text(encoding="utf-8")

    assert 'id="search-context"' in html
    assert 'static/controllers_search_context.js' in html
    assert 'static/controllers_search_context.css' in html
