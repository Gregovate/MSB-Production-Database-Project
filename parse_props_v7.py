...
# Current Version : 2025-11-04  V7.0.0
# (GAL)
...
# ----------------
# V7.0.0 (GAL 25-11-04)
#   • Added parse_prop_groups() for new <PropGroup> elements under <PreviewClass>
#   • Created `groups` table in DB schema
#   • Linked `PreviewId` foreign key to `previews.id`
#   • Updated process_file() to call process_prop_groups()
#   • Maintains full backward compatibility with v6
#
# V6.8.0  (GAL 2025-11-02)
...
def setup_database():
    ...
    cursor.execute("DROP TABLE IF EXISTS groups")      # GAL 25-11-04: new PropGroup table
    ...
    # Create Groups Table (PropGroup under PreviewClass) 2025-11-04 GAL
    cursor.execute("""
    CREATE TABLE groups (
        IntGroupID INTEGER PRIMARY KEY AUTOINCREMENT,
        GroupID TEXT UNIQUE,            -- PropGroup id attribute (GUID)
        Name TEXT,                      -- PropGroup Name
        Tag TEXT,                       -- PropGroup Tag (e.g., "LOR Group-RGB Matrix 20x40")
        Arrangement TEXT,               -- Arrangement="Horizontal"/"Vertical"/etc.
        PreviewResFactor REAL,          -- PreviewResFactor attribute
        PreviewSinglePointPerProp BOOLEAN,  -- "True"/"False" as stored
        EffectBulbSize REAL,            -- EffectBulbSize attribute
        PreviewId TEXT,                 -- FK back to previews.id
        FOREIGN KEY (PreviewId) REFERENCES previews (id)
    )
    """)
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_groups_preview ON groups(PreviewId)")
...
# 2025-11-04 GAL: Process PropGroup elements    -----------------------------
def process_prop_groups(preview_id, root):
    """
    Process <PropGroup> elements that live under <PreviewClass>.
    We care primarily about Tag + Name for reporting, but we also
    capture arrangement and display-related attributes.

    Example:
      <PropGroup id="e8b8ee23-..." Arrangement="Horizontal"
                 PreviewResFactor="1" PreviewSinglePointPerProp="False"
                 EffectBulbSize="1"
                 Tag="LOR Group-RGB Matrix 20x40"
                 Name="RGB Plus Stage 03 Mega Cube">
    """
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()

    for grp in root.findall(".//PropGroup"):
        group_id = grp.get("id")
        name = grp.get("Name")
        tag = grp.get("Tag")

        arrangement = grp.get("Arrangement")
        preview_res_factor = grp.get("PreviewResFactor")
        preview_single_point = grp.get("PreviewSinglePointPerProp")
        effect_bulb_size = grp.get("EffectBulbSize")
        ...
        """, (
            ...
            arrangement,
            preview_res_factor,
            preview_single_point,
            effect_bulb_size,
            preview_id
        ))

        if DEBUG:
            print(f"[DEBUG] Inserted PropGroup: {group_id}  Tag={tag}  Name={name}")
...
def process_file(file_path):
    ...
    if preview is not None:
        ...
        # NEW: capture <PropGroup> records for this preview 2025-11-04 GAL
        process_prop_groups(preview_data["id"], root)
