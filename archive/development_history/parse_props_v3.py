...
def process_file(file_path, conn):
    ...
    for elem in root.iter():
        ...
        if elem.tag == "PropClass":
            ...
            # 2. Shared DisplayName, Single Grid, No MasterPropId
                ...
                # Process the grid parts
                grid_parts = channel_grid.split(',')  # Split the single grid into parts
                ...
                for display_name, group in grouped_props.items():
                    ...
                    props.append({
                        ...
                        "Unknown": unknown,
                        "Color": color,
                        "MaxChannels": int(attributes.get('MaxChannels', 0)),
                        "Tag": attributes.get('Tag', None),
                        "Lights": int(attributes.get('Parm2', 0)),
                        "DimmingCurveName": attributes.get('DimmingCurveName', None),
                        "Segments": int(attributes.get('Parm1', 0)),
                        "Opacity": float(attributes.get('Opacity', 0.0)),
                        "MasterDimmable": attributes.get('MasterDimmable', 'false') == 'true',
                        "PreviewBulbSize": float(attributes.get('PreviewBulbSize', 0.0)),
                        "BulbShape": attributes.get('BulbShape', None),
                        "CustomBulbColor": attributes.get('CustomBulbColor', None),
                        "StartLocation": attributes.get('StartLocation', None),
                        "StringType": attributes.get('StringType', None),
                        "TraditionalColors": attributes.get('TraditionalColors', None),
                        "TraditionalType": attributes.get('TraditionalType', None),
                        "RgbOrder": attributes.get('RgbOrder', None),
                        "SeparateIds": attributes.get('SeparateIds', None),
                        "EffectBulbSize": float(attributes.get('EffectBulbSize', 0.0)),
                        "IndividualChannels": attributes.get('IndividualChannels', '').strip().lower() == 'true',
                        "LegacySequenceMethod": attributes.get('LegacySequenceMethod', None),
                    })
                    ...
                    for prop in group:
                        if prop["id"] != master_prop_id:
                            sub_props.append({
                                "SubPropID": prop["id"],
                                "Name": prop["Name"],
                                "DisplayName": f"{display_name} - {prop['StartChannel']}",
    ...
    cursor.executemany(
        ...
        INSERT OR IGNORE INTO props (
            PropID, Name, DisplayName, DeviceType, MaxChannels, Tag, Network, UID,
