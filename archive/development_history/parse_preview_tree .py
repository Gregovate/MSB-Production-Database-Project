...
def initialize_database():
    ...
    # Create or connect to the LOR database
...
CREATE TABLE fixtures (
    ...
    cursor.execute("""
        ...
        SELECT 
            ...
            f.Lights AS FixtureLights,
            f.Comment AS FixtureComment,
            d.Network AS DMXNetwork,
            d.Start_Unv AS DMXStartUnv,
            d.Start_Channel AS DMXStartChannel,
            d.End_Channel AS DMXEndChannel,
            d.Unknown AS DMXUnknown,
            d.Color AS DMXColor
        FROM props p
        LEFT JOIN fixtures f ON p.PropID = f.MasterPropId
        LEFT JOIN dmxChannels d ON p.PropID = d.PropId
    """)

    # Create a view to filter LOR props
    cursor.execute("""
