...
# Filename: compare_displays_vs_db.py
# Revision history
...
#   GAL 26-02-22  Include inventory_type and display_status in compare pipeline;
#                 carry through to Matches, Sheet_Only, and Needs_Fix tabs.
# ------------------------------------------------------------
...
def load_sheet_names(csv_path: Path) -> pd.DataFrame:
    ...
    Returns DataFrame with columns:
      display_name, display_name_clean, display_name_norm,
      inventory_type, display_status
    """
    ...
    with open(csv_path, "r", encoding="utf-8-sig", newline="") as f:
        ...
        norm_hdrs = [ (h or "").strip().lower().replace("_"," ").replace("-"," ") for h in headers ]
        ...
        # Find optional new columns
        inv_idx = None
        for want in ("inventory type", "inventory_type"):
            if want in norm_hdrs:
                inv_idx = norm_hdrs.index(want)
                break

        status_idx = None
        for want in ("display status", "display_status", "status"):
            if want in norm_hdrs:
                status_idx = norm_hdrs.index(want)
                break
        ...
        for row in r:
            ...
            inv = cells[inv_idx] if inv_idx is not None else ""
            st  = cells[status_idx] if status_idx is not None else ""
    ...
    df = pd.DataFrame({
        "display_name": disp_values,
        "inventory_type": inv_values,
        "display_status": status_values,
    })
    ...
    # Normalize new fields (optional but helpful)
    df["inventory_type"] = df["inventory_type"].astype(str).str.strip().str.upper()
    df["display_status"] = df["display_status"].astype(str).str.strip().str.upper()

    df = df[df["display_name_norm"] != ""].drop_duplicates(subset=["display_name_clean"])

    print(f"[INFO] Using column for display name: '{headers[disp_idx]}'")
    if inv_idx is not None:
        print(f"[INFO] Found inventory_type column: '{headers[inv_idx]}'")
    else:
        print("[WARN] inventory_type column not found in CSV header (will be blank in report)")

    if status_idx is not None:
        print(f"[INFO] Found display_status column: '{headers[status_idx]}'")
    else:
        print("[WARN] display_status column not found in CSV header (will be blank in report)")
...
def run_compare(db_path: Path, csv_path: Path) -> dict:
    ...
    sheet_only = sh_df[~sh_df["display_name_clean"].isin(matches["Exact_Key"])] \
        ...
        [["Sheet_Display_Name","display_name_clean","Normalized_Key","inventory_type","display_status"]] \
