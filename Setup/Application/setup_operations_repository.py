"""PostgreSQL access for Setup organization, scheduling, and field execution."""
...
class SetupOperationsRepositoryError(RuntimeError):
    """Next-pass Setup operation failed safely."""


class SetupOperationsRepository:
    def __init__(self, dsn: str):
        self.dsn = (dsn or "").strip()
        if not self.dsn:
            raise SetupOperationsRepositoryError("Setup PostgreSQL DSN is required")
    ...
    def organization(self) -> dict[str, list[dict[str, Any]]]:
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                ...
                ORDER BY ls.stage_id, ls.scene_name, ls.lor_scene_id
                """
            )
            scenes = [dict(row) for row in cur.fetchall()]
            ...
            scopes = [dict(row) for row in cur.fetchall()]
    ...
    def set_scope(
        ...
        *,
        ...
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM ref.set_setup_task_scope(%s,%s,%s,%s)",
                (email, setup_task_id, stage_id, lor_scene_id),
            )
            row = cur.fetchone()
            conn.commit()
        if row is None:
            raise SetupOperationsRepositoryError("Setup task scope command returned no result")
        return dict(row)
    ...
    def set_dependency(
        ...
        *,
        email: str,
        setup_task_id: int,
        prerequisite_setup_task_id: int,
        dependency_note: str | None,
        active: bool,
    ) -> dict[str, Any]:
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM ref.set_setup_task_dependency(%s,%s,%s,%s,%s)",
                (
                    email,
                    setup_task_id,
            ...
            conn.commit()
        if row is None:
            raise SetupOperationsRepositoryError("Setup prerequisite command returned no result")
        return dict(row)
    ...
    def upsert_work_day(
        ...
        *,
        ...
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM ops.upsert_setup_work_day(%s,%s,%s::date,%s,%s)",
                (email, season_year, work_date, day_status, notes),
            )
            row = cur.fetchone()
            conn.commit()
        if row is None:
            raise SetupOperationsRepositoryError("Setup work-day command returned no result")
        return dict(row)

    def set_work_day_task(
        self,
        *,
        email: str,
        ...
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM ops.set_setup_work_day_task(%s,%s,%s,%s,%s,%s,%s)",
                (
                    email,
                    setup_work_day_id,
            ...
            conn.commit()
        if row is None:
            raise SetupOperationsRepositoryError("Setup scheduled-task command returned no result")
        return dict(row)
        ...
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                ...
                SELECT
                    ...
                    cp.person_name AS completed_by_name,
                    t.task_name,
                    t.task_action_type,
                    t.display_order,
                    t.stage_id,
    ...
    def task_progress(self, setup_session_task_id: int) -> list[dict[str, Any]]:
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT
                    p.setup_task_progress_id,
                    p.setup_session_task_id,
                    p.setup_work_day_id,
                    wd.work_date,
                    p.shift_code,
                    p.crew_count,
                    ...
                    p.completed_units,
                    p.progress_note,
                    p.marks_task_complete,
                    p.recorded_at,
                    actor.person_name AS recorded_by_name
                FROM ops.setup_task_progress p
                LEFT JOIN ops.setup_work_day wd
                  ON wd.setup_work_day_id = p.setup_work_day_id
                LEFT JOIN ref.person actor
                  ON actor.person_id = p.created_by_person_id
    ...
    def field_context(self, *, setup_task_id: int, season_year: int) -> dict[str, list[dict[str, Any]]]:
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute(
                """
                SELECT
    ...
    def record_progress(
        ...
        *,
        email: str,
        setup_session_task_id: int,
        setup_work_day_id: int | None,
        shift_code: str,
        crew_count: int,
        completed_quantity: int | None,
        completed_units: str | None,
        ...
        with self.connect() as conn, conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            conn.commit()
        if row is None:
            raise SetupOperationsRepositoryError("Setup progress command returned no result")
        return dict(row)
