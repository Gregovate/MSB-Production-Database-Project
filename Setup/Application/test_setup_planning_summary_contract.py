...
def test_catalog_exposes_contextual_stage_scene_and_broad_print_launchers() -> None:
    ...
    assert 'setup_planning_summary.js?v=2026-09-25.1' in production
    assert "Print Planning Summary…" in js
    assert "button.className = 'planning-summary-launch-button';" in js
    assert "button.className = 'secondary';" not in js
