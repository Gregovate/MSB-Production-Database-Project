/* ============================================================================
MSB Setup Session — active-person Captain selection hardening
Issue: #122
Status: IMPLEMENTATION CANDIDATE — DO NOT APPLY TO PRODUCTION WITHOUT REVIEW
Revision: 2026-09-08 V0.1.0

Purpose:
  Correct the Captain / knowledge-owner picker so inactive ref.person rows are
  not offered for new reusable-task leadership assignments.

Rules:
  - ref.person remains the canonical person/contact directory.
  - Only active people appear in the Captain candidate projection.
  - The governed Captain write command independently refuses a new/updated
    active assignment to an inactive person.
  - Existing Captain rows remain readable even if a person is later made
    inactive, so durable knowledge/history is not silently hidden or deleted.
  - Removal of an existing Captain row remains allowed for an inactive person.
  - This is the minimum active-person guard. Future People/Skills work may add
    explicit Captain-candidate eligibility without weakening this requirement.
============================================================================ */
...
BEGIN
    IF to_regclass('ref.person') IS NULL
       ...
       OR to_regclass('ref.setup_task_captain') IS NULL THEN
        RAISE EXCEPTION 'Person, Setup task, and Setup task Captain tables are required';
    END IF;
...
/* Manager-only candidate list. Inactive people stay in ref.person for durable
   identity/history but are not eligible for new Captain selection. */
...
AS $function$
    ...
    FROM ref.person p
    WHERE p.active_flag
      AND nullif(btrim(pg_catalog.concat_ws(' ', p.first_name, p.last_name, p.email)), '') IS NOT NULL
...
/* Preserve migration 020's narrow write contract while adding an independent
   database guard. UI filtering alone is not sufficient authorization/validity
   enforcement. */
    ...
    IF coalesce(p_active, true)
       AND NOT EXISTS (
        ...
        RAISE EXCEPTION USING ERRCODE = '22023',
            MESSAGE = 'Selected person is inactive and cannot receive a Setup Captain / knowledge-owner assignment';
    END IF;
