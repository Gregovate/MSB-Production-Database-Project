...
$source = Join-Path $repo 'Setup\Acceptance\setup_stage_scene_material_disposable_acceptance_server.sh'
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
    throw "Required acceptance runner is missing: $source"
}

$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$localBundle = Join-Path $env:TEMP "msb-setup-stage-scene-material-$stamp"
$remoteBundle = "/tmp/msb-setup-stage-scene-material-$stamp"
$runner = Join-Path $localBundle 'setup_stage_scene_material_disposable_acceptance_server.sh'
...
try {
    ...
    Write-Host '========== SETUP STAGE/SCENE MATERIAL DISPOSABLE ACCEPTANCE =========='
    ...
    $remoteRunner = "$remoteBundle/setup_stage_scene_material_disposable_acceptance_server.sh"
    & ssh -tt $Server "chmod 700 '$remoteRunner' && bash -n '$remoteRunner' && bash '$remoteRunner' '$TargetRef' '$CandidateSha' '$remoteBundle'"
    if ($LASTEXITCODE -ne 0) {
        throw "Setup Stage/Scene material disposable acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
