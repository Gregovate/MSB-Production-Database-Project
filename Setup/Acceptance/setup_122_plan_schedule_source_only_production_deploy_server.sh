...
TARGET_REF="main"
EXPECTED_LIVE_SHA="f71f578222b2ab5416fabb298e4d4767a2c8d4b5"
TARGET_SHA="6d9fdabdee953a40b8018562f6c588dd19c5b502"
EXPECTED_VERSION="V0.3.17-performance-trace"

REPORT_DIR="/home/msbadmin/setup-deployment-reports"
STAMP="$(date +%Y%m%dT%H%M%S)"
REPORT="$REPORT_DIR/Setup_122_Plan_Schedule_Source_Only_Production_Deploy_$STAMP.txt"
CANDIDATE_WORKTREE="/tmp/msb-setup-122-plan-schedule-candidate-$STAMP"
DETACHED_PYCACHE="/tmp/msb-setup-122-plan-schedule-detached-$STAMP"
LIVE_PYCACHE="/tmp/msb-setup-122-plan-schedule-live-$STAMP"
NEGATIVE_BODY="/tmp/msb-setup-122-plan-schedule-negative-$STAMP.json"
...
echo "========== SETUP #122 PLAN / SCHEDULE SOURCE-ONLY PRODUCTION DEPLOYMENT =========="
...
cleanup() {
    ...
    if [[ -n "$FINAL_2026" && "$FINAL_2026" != "0" ]]; then
        echo "FAIL: #122 source-only deployment created or observed a real 2026 Setup Session"
        status=96
    fi
...
if [[ "$OLD_HEAD" != "$EXPECTED_LIVE_SHA" ]]; then
    echo "FAIL: live Setup SHA changed from the browser-reviewed pre-deployment state"
...
if [[ "$SETUP_PRE" != *"\"status\":\"ok\""* \
   ...
   || "$SETUP_PRE" != *"\"version\":\"$EXPECTED_VERSION\""* ]]; then
    echo "FAIL: current Setup runtime is not the expected $EXPECTED_VERSION state"
    exit 8
fi

if [[ "$(setup_2026_count)" != "0" ]]; then
    echo "FAIL: a real 2026 Setup Session exists before this source-only deployment"
    exit 9
fi
...
if ! sudo git -C "$REPO_ROOT" merge-base --is-ancestor "$TARGET_SHA" "origin/$TARGET_REF"; then
    echo "FAIL: exact browser-accepted #122 target is not contained in merged origin/main"
    exit 11
fi
if ! sudo git -C "$REPO_ROOT" merge-base --is-ancestor "$OLD_HEAD" "$TARGET_SHA"; then
    echo "FAIL: accepted #122 target is not a forward descendant of live Setup"
    exit 12
fi
...
for asset in \
    'setup_scheduling_board.css?v=2026-09-24.2' \
    'setup_production.js?v=2026-09-24.3' \
    'setup_catalog_effort.js?v=2026-09-24.5' \
    'setup_catalog_dirty_guard.js?v=2026-09-24.5' \
    'setup_scheduling_board.js?v=2026-09-24.3'; do
    if [[ "$ROOT_HTML" != *"$asset"* ]]; then
        echo "FAIL: deployed Setup HTML is missing accepted asset pin: $asset"
        exit 16
    fi
done
echo "ACCEPTED PLAN / SCHEDULE ASSET PINS: PASS"
...
if [[ "$FINAL_2026" != "0" ]]; then
    echo "FAIL: a real 2026 Setup Session exists after #122 source-only deployment"
    exit 19
fi

SUCCESS=1
echo
echo "SETUP_122_PLAN_SCHEDULE_SOURCE_ONLY_PRODUCTION_DEPLOYMENT_PASS"
echo "Scheduling completion: NOT DECLARED"
