...
# pg_restore --no-acl intentionally removes Production ACLs. Rebuild only the
# fieldwiring_app read/execute boundary plus Production PUBLIC function revokes
# from the real catalog ACL entries. Do not infer effective privileges with
# has_*_privilege(), because that proved incomplete for this role.
