...
$bundleName = "msb-setup-167-extra-material-$stamp"
...
try {
    ...
    Write-Host 'All #167 schema/catalog/inventory writes: disposable PostgreSQL clone only'
    Write-Host ''
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Disposable #167 Extra Material acceptance failed with exit code $LASTEXITCODE"
    }
}
finally {
